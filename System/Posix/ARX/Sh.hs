...
newtype Val = Val CString deriving (Eq, Ord, Show, Monoid)
instance IsString Val where fromString = Val . fromString
instance Norm Val     where norm = Val . norm
instance Bytes Val    where bytes (Val c) = bytes c
instance Sh Val       where sh = Esc.bytes . Esc.sh . bytes
...
var bytes        = guard (leading h && Bytes.all body t) >> Just (Var bytes)
 where (h, t)    = (Bytes.head bytes, Bytes.tail bytes)
       body c    = leading c || (c >= '0' && c <= '9')
       leading c = (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || c == '_'


-- | Shell strings with variable substitution.
newtype VarVal = VarVal [Either Var Val] deriving (Eq, Ord, Show)
instance Sh VarVal       where sh (VarVal l) = (mconcat . map sh) l
instance IsString VarVal where fromString = VarVal . (:[]) . Right . fromString


instance Sh (Either Var Val) where sh (Left var)  = mconcat ["\"$",sh var,"\""]
                                   sh (Right val) = sh val

instance Sh [Val] where sh = mconcat . map (mappend " " . sh)
